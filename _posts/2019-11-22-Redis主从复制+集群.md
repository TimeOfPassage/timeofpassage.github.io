...
![redis-master-slave](https://timeofpassage.github.io/static\image\20191122\docker-redis-master-slave.png)

然后看下客户端工具连接情况：

![redis-gui](https://timeofpassage.github.io/static\image\20191122\redis-gui-connection-status.png)
...
2. 进入根目录、创建`sentinel.conf`文件，执行`apt-get update`更新命令，然后`apt-get install vim`安装vim命令，等待`vim`安装完成
   ...
   ![sentinel-conf-vim](https://timeofpassage.github.io/static\image\20191122\docker-edit-sentinel-conf.png)
...
4. 然后启动命令
   ...
   ![sentinel-mode-start-failure](https://timeofpassage.github.io/static\image\20191122\redis-sentinel-start-failure.png)

   设置访问master密码

   ![](https://timeofpassage.github.io/static\image\20191122\sentinel-auth-pass.png)

5. 模拟master宕机，slave自动选举切换成master的角色

   ![](https://timeofpassage.github.io/static\image\20191122\slave-swtich-master.png)